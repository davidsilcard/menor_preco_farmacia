...
class SaoJoaoScraper(BaseScraper):
    def __init__(self):
        super().__init__("https://www.saojoaofarmacias.com.br")
        self.base_domain = "https://www.saojoaofarmacias.com.br"
        self.search_terms = [term.strip() for term in settings.SAO_JOAO_SEARCH_TERMS.split(",") if term.strip()]
    ...
    async def scrape(self):
        async with async_playwright() as p:
            ...
            for index, term in enumerate(self.search_terms):
                search_url = f"{self.base_domain}/{quote_plus(term)}?map=ft"
                print(f"Buscando na Sao Joao: {search_url}")
    ...
    async def _configure_cep(self, page):
        try:
            cep_button = page.get_by_text("Informe seu Cep")
            if await cep_button.is_visible():
                await cep_button.click()
                await asyncio.sleep(1)

                input_cep = page.locator('input[placeholder*="CEP"], input[inputmode="numeric"]').first
                await input_cep.fill(self.cep)
                await asyncio.sleep(1)

                inserir = page.get_by_role("button", name=re.compile("Inserir", re.I))
                if await inserir.is_visible():
                    await inserir.click()
                    await asyncio.sleep(4)
                    print(f"CEP {self.cep} configurado na Sao Joao.")
        except Exception as e:
            print(f"Aviso: configuracao de CEP da Sao Joao falhou ou foi ignorada: {e}")
    ...
    async def _collect_listing_products(self, page, term):
        ...
        hrefs = await page.locator("a[href*='/p']").evaluate_all(
            """els => [...new Set(
                els.map(e => e.href)
                   .filter(h => /-\\d+\\/p(\\?|$)/.test(h) && h.includes('saojoaofarmacias.com.br'))
            )]"""
        )
        ...
        print(f"Sao Joao: {len(products)} produtos coletados para o termo '{term}'.")
        return products
    ...
    async def _enrich_listing_product(self, context, semaphore, product):
        async with semaphore:
            ...
            try:
                ...
                print(f"Aviso: falha ao enriquecer produto da Sao Joao {product['source_url']}: {e}")
                return None
            finally:
    ...
    def _extract_product_detail(self, html, product):
        ...
        manufacturer = self._extract_manufacturer(normalized_body) or brand
    ...
    def _extract_price(self, product_schema, normalized_body):
        ...
        raise ValueError("Preco nao encontrado na pagina da Sao Joao.")
        ...
        if product_schema:
            for key in ("gtin13", "gtin", "gtin14"):
                ...
                if isinstance(value, str):
                    ...
                    if len(digits) in (8, 12, 13, 14) and not digits.startswith("000"):
                        return digits
    ...
    def _extract_manufacturer(self, normalized_body):
        return self._extract_labeled_value(normalized_body, "fabricante")

    def _extract_labeled_value(self, normalized_body, label):
        match = re.search(
            rf"{label}\s+([a-z0-9\s\-/,'.]+?)(?=\s+(?:marca|quantidade|principio ativo|caracteristicas|codigo do produto|fabricante|registro ms|dosagem|ean|codigo de barras|$))",
            normalized_body,
        )
        if match:
    ...
    def save_to_db(self, products):
        if not products:
            print("Nenhum produto da Sao Joao encontrado para salvar.")
            return

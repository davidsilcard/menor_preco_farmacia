...
class DrogaRaiaScraper(BaseScraper):
    ...
    async def scrape(self):
        async with async_playwright() as p:
            ...
            for term in self.search_terms:
                ...
                print(f"Buscando na Droga Raia: {search_url}")
    ...
    async def _collect_listing_products(self, page, term):
        ...
        print(f"Droga Raia: {len(products)} produtos coletados para o termo '{term}'.")
        return products
    ...
    async def _enrich_listing_product(self, context, semaphore, product):
        async with semaphore:
            ...
            try:
                ...
                print(f"Aviso: falha ao enriquecer produto da Droga Raia {product['source_url']}: {e}")
                return None
            finally:
    ...
    def save_to_db(self, products):
        if not products:
            print("Nenhum produto da Droga Raia encontrado para salvar.")
            return

        session = SessionLocal()
        try:
            pharmacy = session.query(Pharmacy).filter_by(slug="droga-raia").first()
            if not pharmacy:
                raise ValueError("Farmacia Droga Raia nao cadastrada. Rode src.init_db primeiro.")
            ...
            session.commit()
            print(f"Sucesso: {len(products)} snapshots de precos da Droga Raia salvos no banco.")
        except Exception as e:
            session.rollback()
            print(f"Erro ao salvar produtos da Droga Raia no banco: {e}")
        finally:
            session.close()

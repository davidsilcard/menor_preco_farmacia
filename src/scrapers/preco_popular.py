...
class PrecoPopularScraper(BaseScraper):
    def __init__(self):
        super().__init__("https://www.precopopular.com.br")
        self.base_domain = "https://www.precopopular.com.br"
        self.search_terms = [term.strip() for term in settings.PRECO_POPULAR_SEARCH_TERMS.split(",") if term.strip()]
    ...
    def _fetch_products_for_term(self, term: str):
        ...
        print(f"Preco Popular: {len(products)} produtos coletados para o termo '{term}'.")
        return products
    ...
    def save_to_db(self, products):
        if not products:
            print("Nenhum produto da Preco Popular encontrado para salvar.")
            return

        session = SessionLocal()
        try:
            pharmacy = session.query(Pharmacy).filter_by(slug="preco-popular").first()
            if not pharmacy:
                raise ValueError("Farmacia Preco Popular nao cadastrada. Rode src.init_db primeiro.")
            ...
            session.commit()
            print(f"Sucesso: {len(products)} snapshots de precos da Preco Popular salvos no banco.")
        except Exception as e:
            session.rollback()
            print(f"Erro ao salvar produtos da Preco Popular no banco: {e}")
        finally:
            session.close()


if __name__ == "__main__":
    scraper = PrecoPopularScraper()
    data = scraper.scrape()
    if data:
